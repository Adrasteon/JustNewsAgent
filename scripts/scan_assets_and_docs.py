...
KEYWORD_RE = re.compile(
    r"(models|common|docs|markdown_docs|\\.json|\\.md|\\.txt|\\.pt|\\.onnx)", re.I
)
...
def scan() -> None:
    ...
    docs_sorted = sorted(
        docs, key=lambda p: p.stat().st_mtime if p.exists() else 0, reverse=True
    )
    ...
    print(
        "Wrote assets:", OUT_ASSETS, "manifest:", OUT_MANIFEST, "latest docs:", OUT_DOCS
    )
