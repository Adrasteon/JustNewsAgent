...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print('Usage: export_cluster_top10_single.py <cluster_id> [count] [outpath]', file=sys.stderr)
        sys.exit(2)
    cluster_id = str(sys.argv[1])
    count = int(sys.argv[2]) if len(sys.argv) > 2 else 10
    outpath = Path(sys.argv[3]) if len(sys.argv) > 3 else Path('logs/crawl/cluster1_top10.md')
