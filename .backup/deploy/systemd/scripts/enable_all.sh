#!/usr/bin/env bash
# PATH wrapper to call the repo's enable_all.sh regardless of CWD
set -euo pipefail
...
ROOT="$(resolve_root)"
SCRIPT="$ROOT/deploy/systemd/enable_all.sh"
if [[ -x "$SCRIPT" ]]; then
  exec "$SCRIPT" "$@"
else
  echo "enable_all.sh not found or not executable at $SCRIPT" >&2
  exit 1
fi
