...
class PracticalNewsReader:
    ...
    async def initialize_option_a_lightweight_llava(self):
        ...
        try:
            ...
            logger.info(
                f"✅ LLaVA-1.5 loaded with "
                f"{memory_usage.get('allocated_gb', 0):.1f}GB GPU memory"
            )
    ...
    async def initialize_option_b_blip2_quantized(self):
        ...
        try:
            ...
            memory_usage = self.get_memory_usage()
            logger.info(
                f"✅ BLIP-2 quantized loaded with "
                f"{memory_usage.get('allocated_gb', 0):.1f}GB GPU memory"
            )
    ...
    async def analyze_image_url(
        ...
        if not prompt:
            prompt = (
                ...
                "Describe what you see and identify any text, headlines, "
                "or news-relevant information. "
        ...
        try:
            ...
            if hasattr(self.processor, 'apply_chat_template'):
                # LLaVA format - use processor directly with image and text
            ...
            # Generate response
            with torch.no_grad():
                # Use safe pad_token_id (avoid None values that can cause issues)
                try:
                    pad_token_id = getattr(self.processor, 'eos_token_id', 2)
                    if pad_token_id is None:
                        pad_token_id = 2  # Common EOS token ID fallback
            ...
            # Decode response
            if hasattr(self.processor, 'apply_chat_template'):
                # LLaVA: decode full output (new format handles tokenization internally)
            ...
            return {
                ...
                "model_used": (
                    "LLaVA-1.5" if hasattr(self.processor, 'apply_chat_template')
                    else "BLIP-2"
                )
            }
...
async def memory_status():
    ...
    return {
        ...
        "memory_usage": memory_usage,
        "model_type": (
            "LLaVA-1.5" if hasattr(newsreader.processor, 'apply_chat_template')
            else "BLIP-2"
        )
    }
...
async def test_practical_approach():
    ...
    try:
        ...
        test_url = (
            "https://cdn.pixabay.com/photo/2023/01/01/12/00/00/"
            "newspaper-7689092_1280.jpg"
        )
    ...
    print("\n" + "=" * 60)
    print(
        "🎯 Conclusion: Practical approach validates user's INT8 insight"
    )
